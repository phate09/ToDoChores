...
OptionScreen = Class(Screen, function( self, lord )
    ...
    self.config = KnownModIndex:GetModConfigurationOptions_Internal(modname, false)
    self.options = {}
...
function OptionScreen:Apply()
    if self:IsDirty() then
        ...
        KnownModIndex:SaveConfigurationOptions(function()
            self:MakeDirty(false)
            TheFrontEnd:PopScreen()
        end, modname, settings, self.client_config)
    else
        self:MakeDirty(false)
        TheFrontEnd:PopScreen()
    end
